#!/usr/bin/env python3
"""
Generate 72 new location pages with full SEO optimization.
Creates: locationData entries, React components, routing, menu items
"""

import os
import json

# All 72 new locations with their data
new_locations = [
...
def generate_description(name, region):
    """Generate SEO meta description (under 160 chars with phone number)"""
    base = f"Shot Blasting {name} - Professional rust removal & surface preparation for {region}. Call 07970 566409"
    if len(base) > 155:
        base = f"Shot Blasting {name} - Professional rust removal & surface preparation. Call 07970 566409"
    return base

def generate_location_data_entry(loc):
    """Generate locationData.ts entry"""
    return f'''  "{loc['slug']}": {{
    name: "{loc['name']}",
    city: "{loc['name']}",
    region: "{loc['region']}",
    latitude: "{loc['lat']}",
    longitude: "{loc['lng']}",
    description: "{generate_description(loc['name'], loc['region'])}",
    url: "https://commercialshotblasting.co.uk/service-areas/{loc['slug']}"
  }}'''
...
def generate_react_component(loc):
    ...
    # Escape any apostrophes in the name for JavaScript strings
...
export default function {component_name}() {{
  ...
  useEffect(() => {{
    const description = locationData["{slug}"]?.description || "Professional shot blasting services in {name_escaped}. Call 07970 566409";
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <LocalBusinessSchema
        name={{locationData["{slug}"]?.name || "{name_escaped}"}}
        city={{locationData["{slug}"]?.city || "{name_escaped}"}}
        region={{locationData["{slug}"]?.region || "{region}"}}
        description={{locationData["{slug}"]?.description || "Professional shot blasting services in {name_escaped}"}}
      ...
      <section className="py-4 bg-gray-50 border-b border-gray-200">
        <div className="container">
          <Breadcrumb items={{[
            ...
            {{ label: "{region}", href: "/service-areas#{region.lower().replace(' ', '-')}" }},
            {{ label: "{name_escaped}", href: "/service-areas/{slug}", isCurrentPage: true }}
          ]}} />
        </div>
      </section>
      ...
      <HeroCarousel className="py-16 md:py-24">
        <div className="max-w-3xl">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-6" style={{{{ fontFamily: "'Playfair Display', serif" }}}}>
            Shot Blasting Services in {name_escaped}
          </h1>
          <p className="text-lg text-white/90 mb-8">
            Commercial Shot Blasting provides professional surface preparation and rust removal services throughout {name_escaped} and {region}. Serving local manufacturers, industrial facilities, and commercial projects with precision blasting solutions.
          </p>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-[#2C2C2C] mb-6" style={{{{ fontFamily: "'Playfair Display', serif" }}}}>
                Your Local Shot Blasting Experts in {name_escaped}
              </h2>
              <p className="text-gray-700 mb-4">
                Commercial Shot Blasting has been serving {name_escaped} businesses, providing professional surface preparation services to the region's leading manufacturers, industrial facilities, and commercial projects.
              </p>
              <p className="text-gray-700 mb-6">
                {name_escaped}'s diverse industrial sector relies on our expertise for precision surface preparation. Whether you need rust removal, paint stripping, or concrete surface profiling, we have the equipment and experience to deliver exceptional results.
              </p>
              <ul className="space-y-3">
                <li className="flex items-start gap-3">
                  <CheckCircle className="w-5 h-5 text-[#2C5F7F] flex-shrink-0 mt-0.5" />
                  <span className="text-gray-700">Same-day quotes for {name_escaped} area projects</span>
                </li>
                <li className="flex items-start gap-3">
                  <CheckCircle className="w-5 h-5 text-[#2C5F7F] flex-shrink-0 mt-0.5" />
                ...
                <li className="flex items-start gap-3">
                  <CheckCircle className="w-5 h-5 text-[#2C5F7F] flex-shrink-0 mt-0.5" />
                  <span className="text-gray-700">Competitive pricing for {name_escaped} businesses</span>
                </li>
              </ul>
            </div>
            <div className="relative">
              <img
                ...
                alt="Professional shot blasting results in {name_escaped}"
        ...
        <div className="container">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-[#2C2C2C] mb-4" style={{{{ fontFamily: "'Playfair Display', serif" }}}}>
              Services Available in {name_escaped}
            </h2>
            <p className="text-gray-600 max-w-2xl mx-auto">
              We provide a complete range of shot blasting and surface preparation services tailored to {name_escaped}'s diverse industries.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {{[
              ...
              {{
                ...
                description: "Complete rust removal solutions for automotive, manufacturing, and infrastructure projects across {name_escaped}.",
                icon: Award
              }},
              {{
        ...
        <div className="container">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4" style={{{{ fontFamily: "'Playfair Display', serif" }}}}>
              Industries We Serve in {name_escaped}
            </h2>
            <p className="text-white/80 max-w-2xl mx-auto">
              Our shot blasting services support a wide range of industries throughout {name_escaped} and {region}.
            </p>
          </div>
      ...
      <section className="py-16 bg-white">
        <div className="container">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-600 max-w-2xl mx-auto">
              View the dramatic transformation our shot blasting services can achieve for your {name_escaped} project.
            </p>
          </div>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-[#2C2C2C] mb-4" style={{{{ fontFamily: "'Playfair Display', serif" }}}}>
              Our Coverage in {name_escaped}
            </h2>
            <p className="text-gray-600 max-w-2xl mx-auto">
              We provide shot blasting services throughout {name_escaped} and the surrounding {region} area.
            </p>
          </div>
          <div className="rounded-lg overflow-hidden shadow-lg">
            <LocationMap
              ...
              zoom={{12}}
              markers={{[{{ lat: {loc['lat']}, lng: {loc['lng']}, title: "{name_escaped}" }}]}}
            />
          </div>
        </div>
      </section>
      ...
      {{/* Nearby Towns */}}
      {{nearbyTownsData["{slug}"] && (
        <section className="py-16 bg-white">
          <div className="container">
            <NearbyTowns
              currentLocation="{name_escaped}"
              towns={{nearbyTownsData["{slug}"] || []}}
            />
          </div>
        </section>
      )}}
      ...
      <section className="py-16 bg-[#2C5F7F]">
        <div className="container text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4" style={{{{ fontFamily: "'Playfair Display', serif" }}}}>
            Ready to Get Started in {name_escaped}?
          </h2>
          <p className="text-white/80 max-w-2xl mx-auto mb-8">
            Contact us today for a free quote on your shot blasting project. We serve all of {name_escaped} and the surrounding {region} area.
          </p>
...
def main():
    ...
    # 1. Generate location data entries
    print("Generating location data entries...")
    location_entries = []
    for loc in new_locations:
        location_entries.append(generate_location_data_entry(loc))
    
    location_data_additions = ",\n".join(location_entries)
    
    # Save location data additions to a file for manual insertion
    with open(os.path.join(base_path, "scripts/location_data_additions.txt"), "w") as f:
        f.write(location_data_additions)
    print(f"Location data entries saved to scripts/location_data_additions.txt")
    ...
    # 3. Generate routing entries
    print("Generating routing entries...")
    route_imports = []
    route_entries = []
    ...
    for loc in new_locations:
        ...
        route_entries.append(f'<Route path="/service-areas/{loc["slug"]}" component={{{component_name}}} />')
    
    with open(os.path.join(base_path, "scripts/route_additions.txt"), "w") as f:
        f.write("// Import statements:\n")
        f.write("\n".join(route_imports))
        f.write("\n\n// Route entries:\n")
        f.write("\n".join(route_entries))
    print("Route additions saved to scripts/route_additions.txt")
    
    # 4. Generate menu entries
    print("Generating menu entries...")
    menu_entries = []
    for loc in new_locations:
        menu_entries.append(f'{{ name: "{loc["name"]}", href: "/service-areas/{loc["slug"]}" }}')
    
    with open(os.path.join(base_path, "scripts/menu_additions.txt"), "w") as f:
        f.write(",\n".join(menu_entries))
    print("Menu additions saved to scripts/menu_additions.txt")
    
    # 5. Generate Areas page entries
    print("Generating Areas page entries...")
    # Group by region
    regions = {}
    for loc in new_locations:
        region = loc['region']
    ...
    areas_entries = []
    for region, locs in sorted(regions.items()):
        for loc in sorted(locs, key=lambda x: x['name']):
            areas_entries.append(f'{{ name: "{loc["name"]}", slug: "{loc["slug"]}", region: "{region}" }}')
    
    with open(os.path.join(base_path, "scripts/areas_page_additions.txt"), "w") as f:
        f.write(",\n".join(areas_entries))
    print("Areas page additions saved to scripts/areas_page_additions.txt")
    
    print(f"\n=== Summary ===")
    print(f"Total locations: {len(new_locations)}")
    print(f"Components created: {len(new_locations)}")
    print(f"Files to update manually:")
    print("  - client/src/data/locationData.ts (add location entries)")
    print("  - client/src/App.tsx (add imports and routes)")
    print("  - client/src/components/Header.tsx (add menu items)")
    print("  - client/src/pages/ServiceAreas.tsx (add area listings)")
